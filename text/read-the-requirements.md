...
それはさておき、ここまで述べたようなケースにおいて、「要件」のような文書が重要になると感じています。PCとインターネットのコモディティ化は、ユーザ全員を「コンテンツ制作者」に昇格させました。別の表現をすれば、以前は執筆者として参考にすべき「表記の問題」だったものを、制作者が考えるべき「組版の問題」に変質させました。「要件」が読者対象にすべきはエンジニアだけではないはずです。
